...
func TestLoadMigrationFilesUsesDirectoryTypeAndFilenameVersion(t *testing.T) {
	...
	ddlFiles := loadMigrationFiles(context.Background(), ddlDir, migrationTypeDDL)
	if len(ddlFiles) != 2 {
	...
	seedFiles := loadMigrationFiles(context.Background(), seedDir, migrationTypeSeed)
	if len(seedFiles) != 2 {
...
func TestLoadMigrationFilesSkipsNonstandardSeedNames(t *testing.T) {
	...
	files := loadMigrationFiles(context.Background(), dir, migrationTypeSeed)
	if len(files) != 1 {
...
func TestResolveVersionKeepsDDLHeaderVersion(t *testing.T) {
	...
	writeMigrationTestFile(t, dir, "00000000_000_baseline.sql", "-- @version 20260902_003\n-- CREATE TABLE demo (id BIGINT);\n")
