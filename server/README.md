...
- 预期错误返回 `common/humax.BusinessError`（HTTP 200 + 非零 code）；未知错误交由
  `humax.Wrap` / `humax.WrapPage` 映射为安全的 HTTP 500。Operation 的成功状态固定为 200，
  仅声明 500 错误状态；参数校验与业务错误仍使用 200 的 Envelope。
