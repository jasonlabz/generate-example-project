...
// controllerImpl 是健康检查模块的控制器实现，持有业务层（service）依赖。
type healthCheckControllerImpl struct {
	service health_check.HealthCheckService
}

// 编译期断言：healthCheckControllerImpl 必须实现 HealthCheckController 接口。
var _ HealthCheckController = (*healthCheckControllerImpl)(nil)
...
func NewHealthCheckController(healthCheckService health_check.HealthCheckService) HealthCheckController {
	return &healthCheckControllerImpl{service: healthCheckService}
}
...
func (c *healthCheckControllerImpl) handle(ctx context.Context, _ *struct{}) (*healthCheckOutput, error) {
